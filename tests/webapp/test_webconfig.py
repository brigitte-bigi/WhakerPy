...
class TestWebSiteData(unittest.TestCase):
    ...
    def test_missing_whakerpy_section_raises(self):
        # JSON without the top-level "WhakerPy" key
        fd, path = tempfile.mkstemp(suffix='.json')
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"pagespath": "html", "pages": {}}, f)
    ...
    def test_missing_pagespath_raises(self):
        # JSON with "WhakerPy" section but without "pagespath"
        fd, path = tempfile.mkstemp(suffix='.json')
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"WhakerPy": {"pages": {}}}, f)
        with self.assertRaises(ValueError):
            WebSiteData(path)
        os.remove(path)
