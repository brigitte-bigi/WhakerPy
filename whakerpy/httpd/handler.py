...
class HTTPDHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        # 1) Normalize path and prepare utilities (this will drop the query part from the path).
        ...
        if hasattr(self.server, 'policy_check') is True:
            allowed, content, status, mime = self.server.policy_check(
                self.path, query_string, self.headers)
